	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=1;i<=excel.rowCount("MasterTestCases");i++){
			...
			if(excel.getData("MasterTestCases", i, 2).equalsIgnoreCase("Y")){
				...
				report=new ExtentReports("C:\\Users\\praveena\\Desktop\\PC Anumala\\Selenium\\StockAccountingHybrid\\Reports\\"+TCModule+FuntionLibrary.generateDate()+".html");
				...
				for(int j=1;j<=excel.rowCount(TCModule);j++){
					...
					if(Function_Name.equalsIgnoreCase("typeAction")){
						FuntionLibrary.typeAction(Locator_Type, Locator_Value, Test_Data);
						test.log(LogStatus.INFO, Description);
						
					}
					if(Function_Name.equalsIgnoreCase("clickAction")){
						FuntionLibrary.clickAction(Locator_Type, Locator_Value);
						test.log(LogStatus.INFO, Description);
					}
					if(Function_Name.equalsIgnoreCase("waitForElement")){
						FuntionLibrary.waitForElement(Locator_Type, Locator_Value, Test_Data);
						test.log(LogStatus.INFO, Description);
					}
					...
					test.log(LogStatus.PASS, Description);
						...
						FileUtils.copyFile(srcFile,new File("C:\\Users\\praveena\\Desktop\\PC Anumala\\Selenium\\StockAccountingHybrid\\Screenshots\\TCModule"+FuntionLibrary.generateDate()+".png"));
